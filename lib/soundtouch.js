...
const getWebAudioNode = function (context, filter, sourcePositionCallback = noop, bufferSize = 4096) {
  ...
  window.__soundTouchNode = node; // Prevent aggressive Garbage Collection on iOS

  // Create a silent dummy oscillator to force iOS to process the node
  const forceProcessOscillator = context.createOscillator();
  const forceProcessGain = context.createGain();
  forceProcessGain.gain.value = 0; // Silent
  forceProcessOscillator.connect(forceProcessGain);
  forceProcessGain.connect(node);
  forceProcessOscillator.start(0);
  ...
  node.onaudioprocess = event => {
    ...
    for (; i < framesExtracted; i++) {
      ...
      left[i] = l;
      right[i] = r;
      if (l !== 0 || r !== 0) isSilent = false;
    }

    processCount++;
    if (processCount % 100 === 0) {
      console.log(`[SoundTouch] Processed 100 frames. Extracted: ${framesExtracted}, isSilent: ${isSilent}`);
    }
  };
  return node;
};
...
class PitchShifter {
  constructor(context, buffer, bufferSize, onEnd = noop) {
    ...
    // Prevent iOS WebKit from optimizing away the ScriptProcessorNode
...
// Expose to window for vanilla JS usage without a bundler
